    ...
    public function expenseVsIncome(){
        ...
        if(!empty($incomes)){
            foreach($incomes as $income){
                $total_income += $income->deposited_amount;
                $month = date('m-y', $income->date);
                if($month_year==$month){
                    $this_month_total_income += $income->deposited_amount;
                }
                if(($now-$income->date)<= (7*24*60*60)){
                    $this_week_total_income += $income->deposited_amount;
                }
                if(($now-$income->date)<= (30*24*60*60)){
                    $this_last_30_total_income += $income->deposited_amount;
                }
